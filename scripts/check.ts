...
async function main() {
  ...
  try {
    ...
    if (!currentVersion) {
      log.warn("No local template version found. Run 'bun template:sync' to initialize.")
      return
    }
    ...
    if (comparison === 0) {
      ...
      log.warn(`⚠️  Local version (${currentVersion}) is newer than latest release (${latestVersion})`)
    } else {
    ...
    log.error(`Failed to check for updates: ${error instanceof Error ? error.message : "Unknown error"}`)
    process.exit(1)
  }
}
