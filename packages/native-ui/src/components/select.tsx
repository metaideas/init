...
import { CheckIcon, ChevronDownIcon, ChevronUpIcon, Icon } from "./icon"
...
function SelectTrigger({
  ...
  return (
    ...
    >
      {children}
      <Icon as={ChevronDownIcon} aria-hidden className="size-4 text-muted-foreground" />
    </SelectPrimitive.Trigger>
  )
}
    ...
    >
      <View className="absolute right-2 flex size-3.5 items-center justify-center">
        <SelectPrimitive.ItemIndicator>
          <Icon as={CheckIcon} className="size-4 shrink-0 text-muted-foreground" />
        </SelectPrimitive.ItemIndicator>
      </View>
...
/**
 * Web-only helper that returns null on native platforms.
 */
...
/**
 * Native-only wrapper that falls back to rendering children directly on web.
 */
