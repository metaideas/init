...
function ToastClose({ className, children, render, ...props }: ToastPrimitive.Close.Props) {
  return (
    <ToastPrimitive.Close
      ...
      {...props}
    >
      {children ?? <Icon.X aria-hidden="true" />}
    </ToastPrimitive.Close>
  )
}
...
function ToastIcon({ type }: { type: string | undefined }) {
  ...
  if (type === "success") {
    icon = <Icon.CircleCheck aria-hidden="true" />
  }
  ...
  if (type === "error") {
    icon = <Icon.OctagonX className="text-destructive" aria-hidden="true" />
  }

  if (type === "loading") {
    icon = <Icon.Loader className="animate-spin motion-reduce:animate-none" aria-hidden="true" />
  }
