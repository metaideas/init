...
export function getVersion(): Effect.Effect<string | null, never, FileSystem.FileSystem> {
  return Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem
...
export function getLatestRelease(): Effect.Effect<ReleaseInfo, VersionCheckFailed> {
...
export function requireInitProject() {
  return Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem
    const exists = yield* fs.exists(TEMPLATE_VERSION_FILE)
    if (!exists) {
...
function getAllFiles(): Effect.Effect<string[], never, FileSystem.FileSystem> {
  return Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem
...
export function replaceProjectNameInProjectFiles(
  projectName: string,
